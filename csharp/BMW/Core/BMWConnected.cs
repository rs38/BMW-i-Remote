...
namespace BMW.Models
{
    public class BMW
    {
        static int tokenExpireTime;
        public static string GetToken()
        {
            using (var db = new BMWStateDb())
            {
                if (db.AppStates.Any())
                {
                    ...
                    if (DateTime.Now > appstate.TokenExpireTime)
                    {
                        ...
                        appstate.TokenExpireTime = DateTime.Now.AddSeconds(tokenExpireTime - 60);
                ...
                else
                {
                    ...
                    {
                        CurrentToken = token,
                        TokenExpireTime = DateTime.Now.AddSeconds(tokenExpireTime - 60)
                    };
        ...
        {
            string response = GetAPIToken("your@email.com", "password");
            
            if (!string.IsNullOrEmpty(response))
            {
                Dictionary<string, string> resp_dict = JsonConvert.DeserializeObject<Dictionary<string, string>>(response);
                tokenExpireTime = int.Parse(resp_dict["expires_in"]);
